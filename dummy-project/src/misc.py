...
def load_cifar(label='dog'):
    ...
    train = torchvision.datasets.CIFAR10(root='./input', train=True, download=True, transform=transforms.ToTensor())
    indices = np.array(train.targets) == labels[label]
    # train.data = np.transpose(train.data[indices, :, :, :], axes=[0, 3, 1, 2])
    train.data = train.data[indices]
    # NCHW 5000 x 3 x 32 x 32

    test = torchvision.datasets.CIFAR10(root='./input', train=False, download=True, transform=transforms.ToTensor())
    indices = np.array(test.targets) == labels[label]
    # test.data = np.transpose(test.data[indices, :, :, :], axes=[0, 3, 1, 2])
